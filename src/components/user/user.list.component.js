...
class UserList extends Component {
  ...
  proceedDelete(){
    axios.delete(BACKEND_URL + `/user/${this.state.deleteId}`)
  ...
  updateStateAfterDelete(id){
    console.log('Deleted!');
    this.setState({
      users: this.state.users.filter(b => b._id !== id)
    })
  }
          ...
          <TableBody>
            {this.state.users.map(user => (
              <TableRow className={classes.row} key={user._id}>
                <CustomTableCell align="left"><img alt='' className={classes.imgSize} src={user.picture?user.picture: 'https://www.motorolasolutions.com/content/dam/msi/images/products/accessories/image_not_available_lg.jpg'}/></CustomTableCell>
                <CustomTableCell align="left">{user.name}</CustomTableCell>
                <CustomTableCell align="left">{user.city}</CustomTableCell>
                <CustomTableCell align="left">{user.province}</CustomTableCell>
                <CustomTableCell align="left">{user.country}</CustomTableCell>
                <CustomTableCell align="left">
                  <Tooltip title="Edit User" aria-label="Edit">
                    <IconButton aria-label="Edit" className={classes.margin} component={Link} to={`/user/edit/${user._id}`}>
                      <EditIcon fontSize="small" style={{color: '#F6277F'}}/>
                    </IconButton>
                  </Tooltip>
                  <Tooltip title="Delete User" aria-label="Delete">
                    <IconButton aria-label="Delete" className={classes.margin} onClick={this.btnDeleteOnClick.bind(this, user._id)}>
